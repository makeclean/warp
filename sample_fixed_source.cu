...
__global__ void sample_fixed_source_kernel(unsigned N, unsigned* rn_bank , float * E, source_point* space){
 ...
 */ 
void sample_fixed_source( unsigned NUM_THREADS, unsigned N, unsigned* rn_bank, float * E, source_point* space){

	unsigned blks = ( N + NUM_THREADS - 1 ) / NUM_THREADS;

	sample_fixed_source_kernel <<< blks, NUM_THREADS >>> (  N, rn_bank, E , space );
